...
async function digest(file) { return sha256((await readFile(file)).toString('base64')); }
function unique(values) { return [...new Set(values.filter(Boolean).map((value) => path.resolve(String(value))))]; }
...
async function discoverExactName(name) {
  ...
  const home = homedir();
  const fastRoots = [path.join(home, 'Desktop'), path.join(home, 'Downloads'), path.join(home, 'Documents')];
  for (const root of fastRoots) {
    if (!(await exists(root))) continue;
    for (const row of await boundedCommand('/usr/bin/find', [root, '-type', 'f', '-name', name], 2200)) add(row);
  }

  if (await exists(home)) {
    for (const row of await boundedCommand('/usr/bin/find', [home, '-type', 'f', '-name', name], 5000)) add(row);
  }
  if (await exists('/Volumes')) {
    for (const row of await boundedCommand('/usr/bin/find', ['/Volumes', '-type', 'f', '-name', name], 4000)) add(row);
  }
  return unique(found);
}
...
async function candidateRootsForResult(resultFile, result, explicitAudioRoot = null) {
  ...
  const first = [...(result.chapters ?? [])].sort((a, b) => Number(a.chapterNumber) - Number(b.chapterNumber))[0];
...
async function verifyResultAudioRoot(result, root) {
  ...
  for (const chapter of chapters) {
    ...
    if (!rel || !expected) return { ok: false, reason: `Chapter ${chapter.chapterNumber} lacks Direct MP3 lineage` };
    let file;
    try { file = safeResolve(root, rel); }
    catch (error) { return { ok: false, reason: error.message }; }
    if (!(await exists(file))) return { ok: false, reason: `Chapter ${chapter.chapterNumber} missing under candidate root` };
    const actual = await digest(file);
    if (actual !== expected) return { ok: false, reason: `Chapter ${chapter.chapterNumber} Direct MP3 digest mismatch` };
  }
  return { ok: true };
}

function lineageReason(priorAudit, result) {
  if (result.progress?.allTenComplete !== true || (result.chapters ?? []).length !== 10) return 'not a complete ten-chapter result';
  if (result.source?.productionPlanDigest !== priorAudit.source?.productionPlanDigest) return 'production-plan digest does not match prior audit';
  if (result.cinematicLockDigest !== priorAudit.source?.cinematicLockDigest) return 'cinematic-lock digest does not match prior audit';
  if (result.book?.sourceHash !== priorAudit.source?.manuscriptSourceHash) return 'manuscript source hash does not match prior audit';
  return null;
}

async function discoverRecoverySource(priorAudit, explicitResult = null, explicitAudioRoot = null) {
  const files = explicitResult ? [path.resolve(explicitResult)] : await discoverExactName('cinematic-rebuild-result.json');
  const diagnostics = [];
  const valid = [];
  ...
  for (const file of files) {
    try {
      ...
      for (const root of roots) {
        ...
        if (check.ok) { matchedRoot = root; break; }
      }
      if (!matchedRoot) {
        diagnostics.push({ file, accepted: false, reason: 'no root contains all 10 digest-matching Direct MP3s', roots: rootDiagnostics });
        continue;
      }
      const fileStat = await stat(file);
      valid.push({ file, result, audioRoot: matchedRoot, mtimeMs: Number(fileStat.mtimeMs ?? 0) });
      diagnostics.push({ file, accepted: true, audioRoot: matchedRoot, resultDigest: result.integrity.resultDigest });
    } catch (error) {
  ...
  if (!valid.length) {
    const error = new Error(
      `No coherent current cinematic result was found for pause recovery. ` +
      `YasReady checked ${files.length} cinematic result candidate(s), required exact production-plan + lock + manuscript lineage, ` +
      `and required all 10 current Direct MP3 digests to verify.`
    );
    error.diagnostics = diagnostics;
    throw error;
  }
  return { selected: valid[0], diagnostics, validCount: valid.length };
}
...
async function recover() {
  ...
  if (!priorAuditPath || !out) {
    throw new Error('Usage: node src/pause-recovery-cli.js recover <prior-audit.json> --out DIR [--result FILE] [--audio-root DIR]');
  }
  ...
  try {
    ...
    await writeFile(path.join(root, 'pause-recovery-failure.json'), JSON.stringify({
