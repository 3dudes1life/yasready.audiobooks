...
export function renderBookOnePauseDeficitAuditHtml(audit) {
  ...
  const chapterCards = audit.chapters.map((chapter) => {
    ...
    const rows = attention.map((row) => {
      ...
      const play = timeSec == null ? '<span class="muted">Unlocalized</span>' : `<button class="play" data-player="audio-${chapter.chapterNumber}" data-time="${timeSec.toFixed(3)}">Play around boundary</button>`;
