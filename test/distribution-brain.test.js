...
test('operator flow advances to distribution after upstream approvals', () => {
  ...
  store.put({ id: 'ab', type: 'audio_bible', projectId: 'p1' });
  store.put({ id: 'ch', type: 'character', projectId: 'p1' });
  store.put({ id: 'va', type: 'voice_assignment', projectId: 'p1', locked: true });
  store.put({ id: 'dp', type: 'director_plan', projectId: 'p1' });
  store.put({ id: 'dc', type: 'director_cue', projectId: 'p1' });
  store.put({ id: 'pp', type: 'production_plan', projectId: 'p1' });
  store.put({ id: 'pj', type: 'production_job', planId: 'pp', status: 'ready' });
  store.put({ id: 'rv', type: 'review_session', projectId: 'p1', status: 'approved', locked: true });
  store.put({ id: 'qa', type: 'qa_run', projectId: 'p1', status: 'approved', locked: true });
  store.put({ id: 'ma', type: 'mastering_plan', projectId: 'p1', status: 'mastered', locked: true });
  const status = buildOperatorFlowStatus(store, 'p1');
