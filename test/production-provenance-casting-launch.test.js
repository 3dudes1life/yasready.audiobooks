...
test('0.14.3.4 release constant is current product provenance', () => {
  assert.equal(YASREADY_AUDIOBOOKS_VERSION, '0.14.3.4');
});
...
test('Casting Launch accepts legacy 0.11.8 prep provenance but emits current launch provenance', () => {
  ...
  assert.equal(launch.provenance.applicationRelease, '0.14.3.4');
