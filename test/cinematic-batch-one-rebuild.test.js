...
function ffmpeg(){const write=async(out,label)=>{await mkdir(path.dirname(out),{recursive:true});await writeFile(out,Buffer.from(label));};return{healthCheck:async()=>({ok:true}),tempo:async(i,o,m)=>{await write(o,`tempo:${m}:${await readFile(i,'utf8')}`);},voiceDepth:async(i,o,{mode})=>{await write(o,`finish:${mode}:${await readFile(i,'utf8')}`);},concat:async(inputs,o)=>{let x='';for(const f of inputs)x+=await readFile(f,'utf8');await write(o,x);},master:async(i,o,profile)=>{await write(o,`master:${profile}:${await readFile(i,'utf8')}`);},analyze:async(file)=>({durationSec:60,sampleRateHz:44100,channels:1,channelLayout:'mono',codec:file.endsWith('.wav')?'pcm_s24le':'mp3',bitrateKbps:file.endsWith('.mp3')?192:null,rmsDb:-20.5,peakDb:-4,noiseFloorDb:-70,leadingSilenceMs:1200,trailingSilenceMs:1200})};}
